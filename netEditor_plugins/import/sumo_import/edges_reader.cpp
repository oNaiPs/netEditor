    ...
    while (!atEnd())
    {
        ...
        if (name()=="edge" && tokenType()==QXmlStreamReader::StartElement)
        {
            ...
            foreach (QXmlStreamAttribute a,attributes())
            {
                ...
                else if (a.name()=="nolanes" || a.name()=="numLanes")
		...
		else if (a.name()=="shape")
		{
            ...
            Q_ASSERT_X(firstNode!="" && secondNode != "","node info", "Could not find node associated with id!");
            ...
            if (seg==NULL)
            {
                segment *s=new segment(pointList.value(firstNode),pointList.value(secondNode),DEFAULT_SEGMENT_DRAWING_STATE);
                s->setID(id);
